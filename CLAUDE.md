...
- **Test de sécurité global (19/08/2026)** — revue du diff `dev`→`main` (rien de neuf, purement cosmétique) + audit code complet (secrets/tokens : aucun trouvé dans le repo ni l'historique git ; `npm audit` propre ; XSS : seul usage de `| safe` est `lesson.content` dans `lesson.njk`, contenu rédigé par l'éditeur via Supabase, pas une entrée publique ; formulaire contact protégé par honeypot Netlify Forms) + tests passifs contre `evolvepoker.eu` en prod (headers CSP/HSTS/X-Frame-Options présents y compris sur les 404, redirection HTTP vers HTTPS active, aucun fichier sensible exposé : `.env`/`.git`/`netlify.toml`/`package.json`/`_data/texts.json` tous en 404). 2 points corrigés dans `netlify/functions/lesson-vote.js` : (1) la vue admin (`GET` sans paramètre `lesson`, qui liste tous les votes) était accessible publiquement sans aucune protection malgré le commentaire "vue admin" — ajout d'un token partagé (var d'env `LESSON_VOTE_ADMIN_TOKEN` à définir côté Netlify ; fail-closed vers 404 tant qu'elle n'est pas définie) ; (2) `getClientIp()` retombait sur le header `x-forwarded-for` (falsifiable par le client) si `x-nf-client-connection-ip` (posé par Netlify, fiable) était absent, ce qui aurait permis de contourner le rate limiting anti-spam (3 votes/15min) en changeant d'IP annoncée à chaque requête — fallback retiré. `console.error` ajouté sur l'échec de `getStore()` pour ne plus avaler l'erreur silencieusement. Tests (`npm test`, 10/10) et build (538 fichiers) vérifiés après les 2 fixes.
- **Mention légale IA sur les leçons (19/08/2026)** — recherche réglementaire : AI Act (art. 50, en application depuis le 2/08/2026) exige une mention pour les textes IA informant le public sur des "questions d'intérêt public" (santé/politique/sécurité cités en exemple), sauf exemption si contrôle éditorial humain + responsabilité éditoriale documentés (§4). Conclusion : le contenu des leçons poker n'entre probablement pas dans ce périmètre, et le process actuel (IA génère, éditeur relit/valide systématiquement avant publication Supabase) remplit de toute façon les conditions de l'exemption → **aucune mention n'était légalement obligatoire**. Décision utilisateur : ajouter quand même une mention par transparence, mais dans la Privacy Policy uniquement (pas sur les 480 pages de leçon, écarté après un aller-retour) — nouvelle section "Origine des contenus pédagogiques" dans `_data/texts.json` (panel `privacy`), avec référence explicite à l'art. 50 AI Act (sert de trace documentée du contrôle éditorial humain en cas de contestation). Bonus corrigé au passage : `.lesson-feedback__label` avait le même bug de contraste WCAG AA déjà vu ailleurs le 19/08 (`--color-text-3` sur `--color-surface` ≈ 2,72:1, insuffisant) → `--color-text-2` (≈ 7,07:1). Build vérifié (538 fichiers).
...
| 🟠 | **Backend votes/likes cassé en prod (à traiter avec la refonte backend)** — `/.netlify/functions/lesson-vote` renvoie 500 (`Store unavailable`, échec de `getStore()`) sur 100% des requêtes en prod, constaté le 19/08/2026 ; échoue fermé (pas de fuite de données), mais likes/dislikes indisponibles côté utilisateur. Root cause non creusée en profondeur : décision du 19/08, le backend votes + le reste du tracking vont être repensés entièrement, pas de valeur à investiguer/patcher l'existant avant cette refonte | Site/Externe |
...
| 🟢 | **Activer `LESSON_VOTE_ADMIN_TOKEN` si la vue admin des votes est un jour utile** — sinon rester en fail-closed (404) ; probablement obsolète une fois le backend votes repensé (cf. tâche ci-dessus) | Site |
